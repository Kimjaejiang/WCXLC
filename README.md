...
| 日期 | 模块 | 修改项 | 说明 |
|---|---|---|---|
| 2026-08-27 | 💰 红包与支付 | **修复私聊发红包「请求不成功」+ 假群红包改「分裂群组假红包」** | `允许领取私聊红包` 原实现 hook 发红包界面强制 `key_type=1`，导致个人会话红包发不出去；现改为**仅对分裂群组假群(`@@chatroom`)注入**，真实私聊/群聊发红包恢复正常，分裂假群仍可发送假红包（功能更名「分裂群组假红包」） |
