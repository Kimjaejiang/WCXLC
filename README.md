...
> 8.0.78 下已实测：96% 的锚点有效，少数功能（分裂群组通话、PipVoip）因微信架构迁移失效，详见同章实测表。
...
## 🧩 常见疑问 · 重装模块后「功能变少 / DEX 缓存不全」

> **症状**：重装模块后，DEX 缓存从 149 个掉到 ~90 个，部分功能「开关开着却没生效」。

### 先确认：这**不是**缓存损坏，也不是功能被删

- 缓存文件位于 `/sdcard/Android/data/<微信包名>/WCXLC/dex_cache/`，**每个需要解析锚点的功能一个 `.json`**（全量约 149 个）。
- 掉到 ~90 是**重装瞬间的中间态**：缓存被清空后，需要重新解析才能补齐。
- **解析本身没有问题** —— 实测「开始适配」后 91 个全部解析成功、0 失败，缓存恢复 149，健康检查报「314 个功能全部正常」。

### 根因：`热更新后重置 DEX 缓存` 开关

`DexCacheManager.init()` 分两条路径处理缓存失效（`dexkit/cache/DexCacheManager.kt`）：

| 触发条件 | 是否清缓存 | 说明 |
| --- | --- | --- |
| **微信版本变化** | ✅ **无条件清空** | 必要的：换版本后混淆类名整体重排，旧锚点可能已不存在 |
| **模块版本变化** | ⚠️ **仅当「热更新后重置 DEX 缓存」为开** | 可选行为，**默认关闭** |

也就是说：**开着这个开关时，每次重装/更新模块都会清空全部缓存**，于是重启后有一批功能处于「缓存未就绪、待重新解析」，表现为功能数量变少。

### 处理

- **推荐：关闭「热更新后重置 DEX 缓存」。** 适配结果是按**微信版本**维护的，模块自身重新编译并不影响锚点有效性，没有理由每次重装都重扫。
  - 关闭后重装模块 → 缓存**保持不变**（日志：`未开启「热更新后重置」，缓存保持不变`）→ 无需重新适配。
  - 微信升级时**仍会照常清缓存重扫**，不受此开关影响。
- **已经掉到 ~90 了怎么办**：打开模块设置里的**适配/自检入口点一次「开始适配」**，几十秒内即可补齐；无需清库、无需重装。

### 已知体验瑕疵（待改）

缓存被清空后的**首次启动不会自动补齐**，需要用户手动点一次「开始适配」：

- `FeaturesLoader.resolveOutdatedInline()` 本意是在启动路径上做同步兜底（让「缓存刚被清空」不再等于「本次启动全废」），但对 `outdated` 数量很大（全量 149）的场景，实测该路径**没有实际执行**（无 `attempting inline dex resolution` 日志），`handleBrokenItems()` 的自动修复对话框也未触发。
- 影响面：**仅体验**（需手动点一次），**不丢功能、不坏数据**。关掉上述开关即可绕开这一整类场景。
...
- **🔍 实测 · 重装模块后 DEX 缓存掉到 ~90：非损坏，是「热更新后重置」开关 + 不自动补齐**
  - 现象：重装模块（含 CI 产物）后，`dex_cache/` 从 **149 个 `.json`** 掉到 **91 个**；日志 `found 58 valid items, 91 outdated items`，随后 32 个功能报 `SKIPPED_INCOMPLETE_CACHE`。
  - 排查结论（**非缓存损坏、非功能被删**）：两版 `IResolveDex` 实现文件数**完全相同**（均 143 个），不存在功能被移除。
  - 根因：`DexCacheManager.init()` 的**路径二** —— 模块版本变化时，若「热更新后重置 DEX 缓存」为开，则 `clearAllCache()` 清空全部缓存；重启后 149 个功能全部落入 `outdated`，只有先前残留的 58 个仍有效。
  - **决定性实测**：点一次「开始适配」后，91 个**全部解析成功、0 失败**，缓存 58 → 121 → 145 → **149 稳定**，健康检查恢复「314 个功能全部正常」。**未做任何清库/重装。**
  - 同时发现**体验瑕疵（待改）**：`FeaturesLoader.resolveOutdatedInline()` 在 `outdated` 达全量（149）时**未实际执行**（无 `attempting inline dex resolution` 日志），`handleBrokenItems()` 的自动修复对话框也未触发 → **必须手动点一次「开始适配」**。仅影响体验，不丢功能、不坏数据。
  - 处理：**关闭「热更新后重置 DEX 缓存」**（该项**默认关闭**）。关闭后重装模块缓存保持不变（日志：`未开启「热更新后重置」，缓存保持不变`）；微信版本变化仍无条件清缓存重扫，不受影响。
  - 详见 [🧩 常见疑问 · 重装模块后「功能变少 / DEX 缓存不全」](#-常见疑问--重装模块后功能变少--dex-缓存不全)。
...
> **图例**（功能特性以本仓库实际代码为准，本仓库为 WCX 下游 fork）：
> - 🛠️ **下游增强/修复** = 本仓库在上游基础上二次修改并已应用（含本仓库新增功能）；条目下方列出变更要点，完整实现细节见上方「🛠️ 下游修改项」
> - 无徽章 = 与上游一致，保持原样（历史上若有本地版本，均已「随 v247」采用上游实现，不再重复标记）
...
- **聊天工具栏** 🛠️：增强聊天输入栏，常用功能一键直达
  - 本地增强（08-26）：相册直选注入显示、收藏入口直达 `FavoriteIndexUI`、点击按实际格子动态定位、图标按实际可用格子排布
- **自动同意好友申请** 🛠️：收到好友申请自动通过，支持延时/随机延时、欢迎语、黑名单（8.0.77 完整打通）
...
- **虚拟视频通话**：自定义视频通话画面
- **对话归拢（文件夹）** 🛠️：将多个对话归拢在一个文件夹内显示，聚合未读、角标与 @提醒
  - 本地系列增强（08-25 ~ 09-05）：`[@全体]`/`[有人@我]` 彩色标签（`atCount` bit24 权威标志 + 关键词兜底双重判定，@所有人不再误判为有人@我）、免打扰未读正确计入文件夹角标（小圆点）、标题/提及/成员等 5 色取色器染色（暗色自动提亮）、按账号隔离配置、文件夹长按菜单（08-29 修复弹出，09-05 8.0.78 重构：容器行直钩 `eu5.s0.g` 注入「移出/移到文件夹」，确定性行解析，一步到成员）、新消息刷新节流防卡死（消息风暴 ≥800ms 合并刷新）、学校通知进入企业会话页
...
- **隐藏对话列表分割线** 🛠️：隐藏会话列表/归拢文件夹/搜索页的分割线，界面更清爽
  - 本地增强（08-29）：会话页分割线 View GONE 锁定、归拢文件夹 9-patch 背景替换（深色取灰/亮色取白）、搜索页 `clipBounds` 裁剪，按 Activity 限定避免误伤其它页面
...
- **显示群成员身份**：群主/管理员标识
- **自定义好友头像** 🛠️：为好友/群聊设置本地图片头像（配合对话归拢文件夹展示）
  - 本地增强（08-25/26）：`content://` 复制到私有目录存 `file://`、旧头像自动迁移、文件名时间戳 + UUID 防缓存残留、`onDisable` 可完整卸载
...
- **清理缓存垃圾**：一键清理微信缓存
- **灰度测试管理器**：手动开关微信灰度功能
- **模块内一键更新** 🛠️：模块首页检测到新版本后一键下载并安装升级
  - 本地增强（08-26/27）：版本检测（12 位时间戳 tag / `ver.txt` 随 artifact 上传）、兼容带 ABI 与不带 ABI 两种 APK 命名、下载后 `content://` 转 cacheDir + 自身 FileProvider 安装、兜底匹配 APK 文件名、海外 CI 镜像 502 时直连官方源
- **省电模式**：降低微信后台耗电
...
| 功能 | 失效锚点 | 严重度 | 说明 |
|------|---------|--------|------|
| **SplitGroupCall** | 7 | 🔴 | 旧 MultiTalk / ILink 控制链**整体迁移**，不能只给 `allowFailure` 就算适配 |
| **PipVoip** | 5 | 🔴 | VoIP 控制已迁向 Flutter（`FlutterVoipPlugin` → `Lf73/t`） |
| **HideContacts** | 3 | 🟡 | 受 MultiTalk / VoIP 迁移影响的部分锚点 |
